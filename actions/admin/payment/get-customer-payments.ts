"use server";

import prisma from "@/lib/prisma";
import { TransactionClient } from "@/app/generated/prisma/internal/prismaNamespace";
import { requireAdmin } from "@/lib/auth/require-admin";

export async function getCustomerPayments({pagination, userId}: {pagination:{page: number }, userId: string}) {
  await requireAdmin();

  try {
    const orders = await prisma.$transaction( async (tx: TransactionClient) =>{
        const AllPayments = await tx.payment.findMany({
            where: {
                userId
            },
            include: {
                order: true,
                user: true
            },

            take: 20,
            skip: (pagination.page - 1) * 20,
            orderBy: {
                createdAt: "desc",
            },
        });
        const totalPayments = await tx.payment.count({
            where: {
                userId
            }
        })

        return {AllPayments: AllPayments || [], totalPayments, username: AllPayments[0]?.user?.full_name, page: pagination.page}
    })

    return orders;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(message);
  }
}
